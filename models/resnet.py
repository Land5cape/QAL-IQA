...
class ResNetAutoEncoder(nn.Module):
    ...
    def __init__(self, configs, bottleneck, mid_f=False):
        super(ResNetAutoEncoder, self).__init__()
        self.mid_f = mid_f
        self.encoder = ResNetEncoder(configs=configs, bottleneck=bottleneck, mid_f=mid_f)
        self.decoder = ResNetDecoder(configs=configs[::-1], bottleneck=bottleneck, mid_f=mid_f)
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, configs, bottleneck=False, mid_f=False):
...
if __name__ == "__main__":
    configs, bottleneck = get_configs("resnet50")
    input = torch.randn((5, 3, 224, 224))


    model = ResNetAutoEncoder(configs, bottleneck, mid_f=True)
    output = model(input)
